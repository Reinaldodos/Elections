...
FA = fa(nfactors = 37, r = MAT, rotate = "varimax")

FA$values %>% data.table(Value=.) %>% rowid_to_column() %>% 
  ggplot(mapping = aes(x=rowid, y = Value))+
  geom_point()+
...
+   A ce moment seulement, la droite se détahce du bloc "centre-droit macroniste"
...
# Conclusion
Pour la suite de l'étude, on conservera donc 8 blocs, afin d'isoler l'abstention. Il est important de pouvoir la conserver comme un bloc indépendnant, sans quoi la droite sera diluée dans le bloc "droite et abstention"
